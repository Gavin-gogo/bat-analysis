...
def run_analysis(uploaded_file, sheet_zx: str, sheet_cx: str, concurrent_threshold: float):
    ...
    return df1, df2, pivot, bat_conc, bird_conc, conc_species_total, None
 
 
def build_excel(df1: pd.DataFrame, df2: pd.DataFrame, pivot: pd.DataFrame,
                bat_conc: int, bird_conc: int, conc_species_total: dict) -> bytes:
...
if uploaded:
    with st.spinner("分析中..."):
        df1, df2, pivot, bat_conc, bird_conc, conc_species_total, err = run_analysis(uploaded, sheet_zx, sheet_cx, threshold)
    ...
    if err:
        ...
        xlsx_bytes = build_excel(df1, df2, pivot, bat_conc, bird_conc, conc_species_total)
